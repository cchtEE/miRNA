...
while IFS='' read -r line || [[ -n "$line" ]]; do
	...
	cp $(find $sample.out -name "results.txt" -o -name "novel.txt" -o -name "novel_mature.fa" -o -name "mature_sense_SA.grouped" -o -name "novel_hairpin.fa" -o -name  "microRNAannotation.txt" -o -name "mature.iso" -o -name "mappingStat.txt" -o -name "isomiR_summary.txt" -o -name "hg38_prim_RNAcentral_sense_SA.grouped" -o -name "logFile.txt" -o -name "reads.annotation") $sample.results
	# Cleanup
	#rm -rf $sample.out &
done < "$1"
